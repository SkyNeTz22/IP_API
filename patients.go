...
func GetNumberOfCaretakers(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	bID := mux.Vars(r)["id"]
	selectStringUsername := fmt.Sprintf("SELECT COUNT(IDIngrijitor) FROM Pacienti WHERE `IDMedic` = '%s'", bID)
	rows, err := db.Query(selectStringUsername)
