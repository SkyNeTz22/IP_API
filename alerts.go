...
func GetAlerts(w http.ResponseWriter, _ *http.Request) {
	...
	for rows.Next() {
		var elem Alerte
		if err := rows.Scan(&elem.IDAlerta, &elem.Gravitate, &elem.Mesaj, &elem.IDSender, &elem.IDPacient); err != nil {
...
func GetAlertByPatientID(w http.ResponseWriter, r *http.Request) {
	bID := mux.Vars(r)["id"]
	w.Header().Set("Content-Type", "application/json")
	selectStringId := fmt.Sprintf("SELECT * FROM medassist_db.Alerte WHERE `IDPacient` = '%s'", bID)
	rows, err := db.Query(selectStringId)
	if err != nil {
		panic(err)
	}
...
func CreateAlert(w http.ResponseWriter, r *http.Request) {
	...
	bIDPacient := r.FormValue("IDPacient")
	insertStringAlerte := fmt.Sprintf("INSERT INTO medassist_db.Alerte (`Gravitate`,`Mesaj`, `IDSender`, `IDPacient`) VALUES ('%s', '%s', '%s', '%s')", bGravitate, bMesaj, bIDS, bIDPacient)
...
func UpdateAlert(w http.ResponseWriter, r *http.Request) {
	...
	bIDS := r.FormValue("IDSender")
	bIDPacient := r.FormValue("IDPacient")
	updateStringAlerte := fmt.Sprintf("UPDATE medassist_db.Alerte SET `Gravitate` = '%s', `Mesaj` = '%s', `IDSender` = '%s', `IDPacient` = '%s' WHERE `IDAlerta` = '%s'", bGravitate, bMesaj, bIDS, bIDPacient, bID)
	_, err := db.Exec(updateStringAlerte)
	if err != nil {
		panic(err)
	} else {
		fmt.Println("Actualizarea s-a efectuat cu succes! Urmatoarele date au fost actualizate : ", bGravitate, bMesaj, bIDS, bIDPacient)
	}
}
